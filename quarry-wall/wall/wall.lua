...
function cmd.resume(args)
  local yes = false
  for _, a in ipairs(args or {}) do if a == "-y" then yes = true end end
  ...
  -- A turtle that stopped part way through is almost always still holding
  -- something, and crafting needs it empty. Do that here rather than making
  -- it a separate step you have to remember.
  build.returnSpareFuel(cfg)
  local emptied, eerr = build.park(cfg)
  if not emptied then
    die((eerr or "could not clear the turtle") ..
        " -- empty the overflow chest and try again")
  end
